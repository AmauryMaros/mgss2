...
if run_pca :
    ...
    df1 = mgcsts_samples[mgcsts_samples['mgCST'].isin(range(mgCST1[0],mgCST1[1]+1))]
    df1.loc[:,'label'] = "GroupA"
    df2 = mgcsts_samples[mgcsts_samples['mgCST'].isin(range(mgCST2[0],mgCST2[1]+1))]
    ...
    fig,f = plt.subplots()
    f = sns.scatterplot(x=pca_df['PC1'], y=pca_df['PC2'], hue=mgCST, palette=list(new_legend))
    plt.xlabel('PC1 : ' + str(round(explained_variance[0]*100,2)) + "%")
    plt.ylabel('PC2 : ' + str(round(explained_variance[1]*100,2)) + "%")
    new_patch = []
